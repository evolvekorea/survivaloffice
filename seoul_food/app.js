...
function goToStep(step, type = null) {
    if (currentStep) {
        document.getElementById(currentStep).classList.remove('active'); // 현재 단계에서 active 클래스를 제거
    }
    document.getElementById(step).classList.add('active'); // 새로운 단계에 active 클래스 추가
    currentStep = step; // 현재 화면 단계 업데이트
...
function goBack(step) {
    document.getElementById(currentStep).classList.remove('active'); // 현재 단계에서 active 클래스를 제거
    document.getElementById(step).classList.add('active'); // 이전 단계로 전환
    currentStep = step; // 현재 화면 단계 업데이트
}
...
function toggleButton(buttonId, input) {
    ...
    button.disabled = input.value.trim() === ''; // 입력값이 없으면 버튼 비활성화, 입력값이 있으면 버튼 활성화
}
...
function saveDistrict() {
    ...
    if (districtInput !== '') {
        selectedDistrict = districtInput; // 구를 저장
        goToStep('step-2'); // 다음 단계로 이동
    }
}
...
function showResult(type) {
    ...
    if (type === 'food') {
        ...
        searchValue = parseInt(document.getElementById('price').value, 10);
    }
            ...
            if (filteredStores.length === 0) {
                ...
                document.getElementById('result-content').innerHTML = filteredStores.map(store => `
                    <div class="result-item">
                        ...
                        ${store.foods.filter(food => 
                            (type === 'food' ? food.foodName === searchValue : food.price <= searchValue)
                        ).map(food => `
