(() => {
  ...
  const scenes = {
    opening:{image:'office-dusk.webp',alt:'해 질 무렵 텅 빈 사무실과 복도의 수상한 경비원',location:'17층 사무실',mood:'calm',title:'금요일 오후 5시 47분',text:[
      ...
      '컴퓨터의 종료 버튼을 누르려던 순간, 복도 조명이 한꺼번에 꺼집니다.',
      '<span class="broadcast">[사내 긴급방송] 전 임직원은 현재 위치에서 대기해 주십시오.</span>',
      '방송은 거친 잡음과 함께 끊기고, 유리문 너머에 경비원 한 명이 움직이지 않은 채 서 있습니다.'
    ],next:'door'},
    door:{image:'glass-door.webp',alt:'유리문을 두드리는 지수와 뒤에서 다가오는 좀비 경비원',location:'17층 출입문',mood:'danger',title:'문밖의 지수',timed:7,timeout:2,text:[
      ...
      {label:'지수에게 뒤를 보라고 소리친다',effect:{trust:1,rescued:1,flags:{jisu:true,pantryHint:true}},flash:'지수가 탕비실로 피했습니다',next:'plan'}
    ]},
    plan:{image:'office-dusk.webp',alt:'어두워진 사무실 안에 모인 생존자들',location:'17층 사무실',mood:'suspense',title:'두 개의 메시지',text:[
      '사무실에 남은 사람은 당신과 김 대리, 박 과장, 최 부장뿐입니다.',
      '<span class="message">익명 · B2 서버실로 오세요. 엘리베이터는 절대 타지 마세요.</span>',
      '최 부장은 옥상에 구조 헬기가 올 거라고 주장합니다. 동시에 탕비실 쪽에서 냉장고 문이 닫히는 소리가 들립니다.'
    ...
    pantry:{image:'pantry.webp',alt:'어두운 탕비실과 바닥에 앉아 있는 연구원',location:'17층 탕비실',mood:'suspense',title:'냉장고 안의 비밀',text:[
      '바닥에는 회사가 시험 배포한 집중력 음료가 흩어져 있습니다.',
      '냉장고 옆 연구원이 팔을 숨긴 채 속삭입니다.',
      '<strong>“마신 사람들은 소리에 반응합니다. 사내 단말기의 고주파 때문이에요.”</strong>'
    ...
    afterPantry:{image:'pantry.webp',alt:'탕비실에서 다음 이동을 준비하는 생존자',location:'17층 탕비실',mood:'suspense',title:'신호를 끄는 방법',text:[
      '연구원의 설명에 따르면 감염자들의 공격성을 키우는 송신기는 대표실에 있습니다.',
      '원인을 밝힐 서버실, 신호를 끌 대표실, 탈출 가능성이 있는 옥상. 시간이 빠르게 흐릅니다.'
    ...
    stair:{image:'stairwell.webp',alt:'붉은 비상등이 켜진 계단과 좀비가 된 인사팀장',location:'14층 비상계단',mood:'danger',title:'퇴사 면담',timed:6,timeout:1,text:[
      ...
      {label:'박 과장을 앞세운다',effect:{trust:-2,health:5,flags:{usedPark:true}},flash:'동료 신뢰가 낮아졌습니다',next:'rooftop'}
    ]},
    rooftop:{image:'rooftop.webp',alt:'밤의 옥상에서 맞은편 건물로 이어지는 케이블을 바라보는 생존자들',location:'건물 옥상',mood:'wind',title:'오지 않는 헬기',text:[
      '옥상에는 헬기가 없습니다. 최 부장이 본 문자는 6개월 전 재난 훈련 때 받은 것이었습니다.',
      '맞은편 건물 전광판에는 대피소가 곧 닫힌다는 경고가 깜빡입니다.',
      '두 건물 사이에는 점검용 케이블이 흔들리고 있습니다.'
    ],question:'마지막 기회를 선택하세요.',choices:[
      {label:'케이블을 타고 맞은편 건물로 이동한다',effect:{health:-18,minute:3,flags:{cable:true}},next:'escape'},
      {label:'옥상에서 구조 신호를 보내며 기다린다',effect:{minute:8},next:'ending:overtime'},
    ...
    server:{image:'server-room.webp',alt:'붉고 푸른 조명의 서버실과 유리문 밖의 좀비들',location:'지하 2층 서버실',mood:'tech',title:'집중력 음료 프로젝트',text:[
      ...
      '서버에는 음료를 마신 직원 명단과 고주파 실험 기록이 남아 있습니다.',
      '<strong class="impact">감염 대상자 47명.</strong> 자료를 전송하려면 감염자들이 모인 시설관리실의 발전기를 켜야 합니다.'
    ...
    escape:{image:'service-exit.webp',alt:'서비스 출구에서 카드키를 대는 주인공과 뒤따르는 동료들',location:'직원 전용 출구',mood:'escape',title:'오후 6시 26분',text:[
      '서쪽 대피소가 닫히기까지 몇 분 남지 않았습니다.',
      '출구 앞 감염자들이 천천히 가까워집니다. 지금까지 얻은 물품과 동료만이 마지막 수단입니다.'
  ...
  const endings = {
    allSaved:{image:'survivor-ending.webp',title:'모두의 퇴근',text:'신호를 끄고 증거를 전송한 당신은 동료들과 함께 대피소에 도착했습니다. 모두가 안도한 순간 단체 메시지가 도착합니다.<br><strong>“월요일 오전 8시, 전 직원 비상대책회의.”</strong><br>회사는 망했지만 월요일은 옵니다.',mood:'ending'},
    truth:{image:'survivor-ending.webp',title:'회사가 묻으려 한 것',text:'확보한 자료가 외부로 전달되며 사건의 원인이 세상에 알려집니다. 기자가 처음부터 회사를 의심했느냐고 묻자 당신은 대답합니다.<br><strong>“월급에 비해 복지가 너무 좋았습니다.”</strong>',mood:'ending'},
    ...
    overtime:{image:'server-room.webp',title:'최후의 야근자',text:'구조를 기다렸지만 아무도 오지 않았습니다. 자정이 지나자 출퇴근 시스템이 자동으로 퇴근을 기록합니다.<br><strong>금일 근무시간 15시간 32분<br>연장근무 사유를 입력해 주세요.</strong>',mood:'failure'},
  ...
  function showChoices(scene){
    ...
    $('#choicePanel').hidden=false;audio.sfx(scene.timed?'alert':'next');
    if(scene.timed){$('#timerTrack').hidden=false;timerStart=performance.now();const duration=scene.timed*1000;const timeoutChoice=scene.choices[scene.timeout]||valid[0];timerId=setInterval(()=>{const left=Math.max(0,duration-(performance.now()-timerStart));$('#choiceTimer').textContent=`${Math.ceil(left/1000)}초`;$('#timerFill').style.transform=`scaleX(${left/duration})`;if(left<=0){clearTimer();const button=[...wrap.children].find((_,i)=>valid[i]===timeoutChoice)||wrap.lastElementChild;choose(timeoutChoice,button,wrap)}},80)}
  }
  ...
  function showScene(id){
    ...
    const delay=Math.min(2700,850+scene.text.length*420);if(scene.choices)setTimeout(()=>showChoices(scene),delay);else{$('#continueButton').hidden=false}
    if(scene.choices){$('#continueButton').hidden=true;const first=scene.choices.find(c=>!c.require||c.require(state));if(first&&scenes[first.next])preload(scenes[first.next])}
  }
  ...
  $('#continueButton').addEventListener('click',()=>{const scene=scenes[currentScene];if(!scene?.next)return;state.sceneNumber++;showScene(scene.next)});
